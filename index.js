...
app.post("/events", authMiddleware, (req, res) => {
  ...
  if (!name || !date || !time) {
    return res.status(400).send("Name, date, and time are required.")
  }
...
if (process.argv[2] === "test") {
  ...
  ;(async () => {
    ...
    const fakeRegRequest = { body: { username: "testuser", password: "testpass" } }
    const fakeRegResponse = {
      status(code) {
    ...
    await new Promise((resolve) => {
      app.handle({ method: "POST", url: "/register", body: fakeRegRequest.body }, fakeRegResponse, resolve)
    })
    assert.strictEqual(users.length, 1, "User registration failed")
    let token
    await new Promise((resolve) => {
      app.handle(
        { method: "POST", url: "/login", body: fakeRegRequest.body },
        {
          send(data) {
            token = data.token
            resolve()
    ...
    const fakeEventRequest = { body: eventData, headers: { "x-token": token } }
    let eventResponse
    await new Promise((resolve) => {
      app.handle(
        { method: "POST", url: "/events", body: fakeEventRequest.body, headers: fakeEventRequest.headers },
        {
          status(code) {
            this.statusCode = code
            return this
    ...
    assert.strictEqual(eventResponse.message, "Event created", "Event creation failed")
    let eventsResponse
    await new Promise((resolve) => {
      app.handle(
        { method: "GET", url: "/events", headers: { "x-token": token }, query: { sortBy: "date" } },
        {
          send(data) {
            eventsResponse = data
            resolve()
    ...
    assert.ok(Array.isArray(eventsResponse) && eventsResponse.length === 1, "View events failed")
